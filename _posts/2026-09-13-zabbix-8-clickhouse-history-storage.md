...
The only lever that actually worked was blunt: periodically drop and recreate the history tables to force an immediate reclaim. We ended up doing that roughly every six months. In practice that meant scheduling a maintenance window on a production system every team depended on for alerting, and accepting that we were throwing away months of historical trend data we might have wanted for capacity planning, just to buy back disk space the housekeeper should have been reclaiming on its own. It worked, but it was an operational tax we paid on a schedule, not a fix, and it was a quiet admission that the housekeeper simply isn't a viable retention mechanism once you're ingesting at that volume.
